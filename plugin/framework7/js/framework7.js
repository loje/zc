        ...
        var Messages = function (container, params) {
            var defaults = {
                ...
                messageTemplate:
                    ...
                    '{{/if}}' +
                    '<div class="letter letter-{{type}} {{#if hasImage}}letter-pic{{/if}} {{#if avatar}}letter-with-avatar{{/if}} {{#if position}}letter-appear-from-{{position}}{{/if}}">' +
                        '{{#if name}}<div class="letter-name">{{name}}</div>{{/if}}' +
                        '<div class="letter-text">{{text}}{{#if date}}<div class="letter-date">{{date}}</div>{{/if}}</div>' +
                        '{{#if avatar}}<div class="letter-avatar" style="background-image:url({{avatar}})"></div>{{/if}}' +
                        '{{#if label}}<div class="letter-label">{{label}}</div>{{/if}}' +
                    '</div>'
            };
            params = params || {};
            for (var def in defaults) {
            ...
            m.layout = function () {
                ...
                m.container.find('.message').each(function () {
                    var message = $(this);
                    if (message.find('.letter-text img').length > 0) {
                        var childNodes = message.find('.letter-text')[0].childNodes;
                        var onlyPic = true;
                        ...
                        if (onlyPic) message.addClass('letter-pic');
                        else message.removeClass('letter-pic');
                    }
                    if (message.find('.letter-avatar').length > 0) message.addClass('letter-with-avatar');
                });
                m.container.find('.message').each(function () {
                    var message = $(this);
                    var isSent = message.hasClass('letter-sent');
                    var next = message.next('.letter-' + (isSent ? 'sent' : 'received'));
                    var prev = message.prev('.letter-' + (isSent ? 'sent' : 'received'));
                    if (next.length === 0) {
                        message.addClass('letter-last letter-with-tail');
                    }
                    else message.removeClass('letter-last letter-with-tail');
        
                    if (prev.length === 0) {
                        message.addClass('letter-first');
                    }
                    else message.removeClass('letter-first');
        
                    if (prev.length > 0 && prev.find('.letter-name').length > 0 && message.find('.letter-name').length > 0) {
                        if (prev.find('.letter-name').text() !== message.find('.letter-name').text()) {
                            prev.addClass('letter-last letter-with-tail');
                            message.addClass('letter-first');
                        }
                    }
                });
            ...
            var notificationTemplate = app.params.notificationTemplate || 
                ...
                '<li class="notification-item notification-hidden">' +
                    '<div class="item-content">' +
                        '{{#if material}}' +
                            '<div class="item-inner">' +
                                '<div class="item-title">{{js "this.letter || this.title || this.subtitle"}}</div>' +
                                '{{#if ../button}}{{#button}}' +
                                '<div class="item-after">' +
                        ...
                        '{{else}}' +
                            ...
                            '<div class="item-inner">' +
                                ...
                                '{{/if}}' +
                                '{{#if letter}}' +
                                '<div class="item-text">{{letter}}</div>' +
                                '</div>' +
                            '{{/if}}' +
                        '{{/if}}' +
                    '</div>' +
